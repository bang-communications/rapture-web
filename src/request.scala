...
abstract class Request {
  ...
  /** The path of the script, as a List of Strings */
  lazy val path : SimplePath = new SimplePath((servicePathString+remainderString).replaceAll("^\\/", "").split("\\/").filter(_ != "").reverse) {
    val params = Nil
  }
